...
These tickets are based on the README MVP scope plus gaps in current backend business logic and migrations.

Legend:
- Priority: P0 critical, P1 high, P2 medium, P3 low
- Labels: MVP, Pre-MVP, Optional, TechDebt, and domain labels (Auth, Import, Finance, Normalization, Insights, Foundation, BYOS, Payments, Platform)

## Auth and Users

### AUTH-001: Wire user profile endpoints and align user schema
- Priority: P1
- Labels: MVP, Auth, TechDebt
- Problem: `UserRepo` queries columns not present in `users`, and `UserRepo`/`UserHandler` are not initialized.
- Subtasks:
...
- Acceptance: GetUser/UpdateUser RPCs return without SQL errors and match the schema.

### AUTH-002: Expose verification + password reset flows
- Priority: P1
- Labels: MVP, Auth
- Problem: verification and password reset flows exist in `AuthService` but no RPCs expose them.
- Subtasks:
...
- Acceptance: RPCs validate inputs and update `user_tokens`/`user_sessions` as expected.

### AUTH-003: Consolidate session/token revocation
- Priority: P1
- Labels: MVP, Auth, TechDebt
- Problem: auth uses `user_sessions` while user repo revokes `refresh_tokens`/`sessions`.
- Subtasks:
...
- Acceptance: all revocation uses a single session system.

### AUTH-004: Update auth email templates to Echo branding
- Priority: P2
- Labels: MVP, Auth
- Problem: templates reference unrelated product features.
- Subtasks:
  - [ ] Rewrite templates in `internal/domain/auth/service/email_service.go`.
  - [ ] Confirm routes match frontend paths from `FRONTEND_URL`.
- Acceptance: email copy matches Echo product language and links.

## Ingestion and Imports

### IMP-001: Implement UploadUserFile storage + retrieval
- Priority: P1
- Labels: MVP, Import
- Problem: UploadUserFile/GetUserFile/ListUserFiles RPCs are unimplemented.
- Subtasks:
...
- Acceptance: uploaded files are stored and retrievable with metadata.

### IMP-002: Implement import job lifecycle APIs
- Priority: P1
- Labels: MVP, Import
- Problem: CreateImportJob/GetImportJob/ListImportJobs are unimplemented.
- Subtasks:
...
- Acceptance: jobs show status transitions and progress counters.

### IMP-003: Save and reuse CSV mappings
- Priority: P1
- Labels: MVP, Import
- Problem: mappings are detected but not persisted or reused.
- Subtasks:
...
- Acceptance: known fingerprints auto-import without re-mapping.

### IMP-004: Make CSV import idempotent
- Priority: P1
- Labels: MVP, Import, TechDebt
- Problem: duplicate `external_id` fails COPY for re-imports.
- Subtasks:
...
- Acceptance: re-importing the same file does not fail and reports duplicates.

### IMP-005: XLSX/document import v1
- Priority: P1
- Labels: MVP, Import
- Problem: only CSV ingestion exists; `documents` table is unused.
- Subtasks:
...
- Acceptance: XLSX uploads produce transactions and document metadata.

## Core Finance API

### FIN-001: Accounts CRUD (create/list)
- Priority: P0
- Labels: MVP, Finance
- Problem: accounts table exists but no RPCs or repo.
- Subtasks:
...
- Acceptance: accounts can be created with currency/type and listed per user.

### FIN-002: Categories CRUD (create/list)
- Priority: P0
- Labels: MVP, Finance
- Problem: categories table exists but no RPCs or repo.
- Subtasks:
...
- Acceptance: categories persist and list per user.

### FIN-003: Transactions list endpoint
- Priority: P0
- Labels: MVP, Finance
- Problem: ListTransactions RPC is unimplemented; repo exists.
- Subtasks:
...
- Acceptance: list endpoint returns total and filtered rows with stable ordering.

### FIN-004: Goals CRUD + pacing
- Priority: P1
- Labels: MVP, Finance
- Problem: goals table exists but no RPCs or logic.
- Subtasks:
...
- Acceptance: goals return progress and pacing metadata.

### FIN-005: Recurring subscriptions list
- Priority: P1
- Labels: MVP, Finance
- Problem: recurring_subscriptions table exists but no RPCs.
- Subtasks:
...
- Acceptance: endpoint returns recurring items per user.

## Normalization and Overrides

### NORM-001: Merchant normalization pipeline
- Priority: P1
- Labels: MVP, Normalization
- Problem: imports only clean description; `merchant_name` is unused.
- Subtasks:
...
- Acceptance: normalized merchant names are stored and can be overridden.

### NORM-002: Categorization rules + user overrides
- Priority: P1
- Labels: MVP, Normalization
- Problem: category mapping from CSV is ignored and no override system exists.
- Subtasks:
...
- Acceptance: overrides persist and show up in insights.

## Insights and Wrapped

### INS-001: Spending overview endpoint
- Priority: P0
- Labels: MVP, Insights
- Problem: MVP needs top categories/merchants and trends; no API exists.
- Subtasks:
...
- Acceptance: endpoint returns MTD vs last month, top categories/merchants, and trend deltas.

### INS-002: Monthly insights generator + API
- Priority: P1
- Labels: MVP, Insights
- Problem: `monthly_insights` table exists but unused.
- Subtasks:
  - [ ] Build a monthly insights job (new service/worker).
  - [ ] Store results in `monthly_insights`.
  - [ ] Add RPC to fetch insights for a month.
- Acceptance: insights include highlights and one action prompt.

### INS-003: Mini-wrapped summaries
- Priority: P1
- Labels: MVP, Insights
- Problem: `wrapped_summaries` table exists but unused.
- Subtasks:
...
- Acceptance: wrapped payload returns share-safe cards.

### INS-004: Subscription detection job
- Priority: P1
- Labels: MVP, Insights
- Problem: recurring charge detection is a core MVP feature but missing.
- Subtasks:
...
- Acceptance: detection job populates recurring subscriptions for active users.

## Foundation and Goals

### FDN-001: Net worth entries + runway metric
- Priority: P1
- Labels: MVP, Foundation
- Problem: net worth tables exist but no API or calculations.
- Subtasks:
...
- Acceptance: endpoint returns assets, liabilities, net, and runway.

### GOAL-002: Goal burn rate insight
- Priority: P2
- Labels: Pre-MVP, Goals
- Problem: README calls for burn-rate pacing but no logic.
- Subtasks:
...
- Acceptance: goals show burn-rate projections.

## BYOS and Sharing

### BYOS-001: Spreadsheet template upload + field mapping
- Priority: P2
- Labels: MVP, BYOS
- Problem: spreadsheet_templates/mappings tables exist but no APIs.
- Subtasks:
...
- Acceptance: mappings persist and can be retrieved for export.

### BYOS-002: Export computed views (shareable/read-only)
- Priority: P2
- Labels: MVP, BYOS
- Problem: BYOS requires export but no output path exists.
- Subtasks:
...
- Acceptance: exports are read-only and share-safe.

## Payments (Optional)

### PAY-001: Stripe Checkout + entitlements (feature-flagged)
- Priority: P3
- Labels: Optional, Payments
- Problem: payments tables exist but no flows.
- Subtasks:
...
- Acceptance: paid users receive premium entitlement and can be revoked on cancel.

## Platform and Config

### PLAT-001: Make Gemini config optional
- Priority: P0
- Labels: MVP, Platform, TechDebt
- Problem: app requires GEMINI env vars even though AI is post-MVP.
- Subtasks:
